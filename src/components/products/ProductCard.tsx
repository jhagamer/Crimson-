...
export default function ProductCard({ product, onAddToCart }: ProductCardProps) {
  ...
  const isSale = product.tag?.toLowerCase().includes('save') || product.tag?.toLowerCase().includes('sale');
        ...
        <div className="aspect-square relative w-full overflow-hidden rounded-t-xl bg-secondary/20"> {/* Added a light background for letterboxing */}
          ...
          {product.tag && (
            <Badge 
              ...
              className={`absolute top-2 left-2 text-xs px-2 py-1 rounded-md ${isSale ? 'bg-red-500 text-white' : ''}`}
            >
              {product.tag}
            </Badge>
          )}
          <Button 
            ...
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); console.log("Wishlist clicked for", product.name);}} 
          >
